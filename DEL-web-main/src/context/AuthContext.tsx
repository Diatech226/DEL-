...
const refreshDelUser=useCallback(async()=>{if(!isLoaded||!isSignedIn){setDelUser(null);return;}setLoading(true);setError(null);try{const token=await getToken();const synced=unwrapData(await apiPost('/api/auth/clerk/sync', undefined, { token: token || undefined }));const current=unwrapData(await apiGet('/api/auth/clerk/me', { token: token || undefined }));setDelUser((current as any)?.user||(synced as any)?.user||current||synced);}catch(e){setError(getErrorMessage(e));setDelUser(null);}finally{setLoading(false);}},[getToken,isLoaded,isSignedIn]);
